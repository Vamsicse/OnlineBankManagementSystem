...
public class SavingsAccountController {
	...
	public ChangesStatus withdrawMoney(int account_id, int customer_id_by,
		...
		try {
			...
			if (newAmount < BusinessRules.SAVING_ACCOUNT_MIN_AMOUNT_TO_LEFT) {
	...
	public ChangesStatus transferMoney(int account_id_from, int account_id_to,
			int customer_id_by, int amount) throws BelowMinimumBalanceException {
		...
		try {
			...
			if (newAmount_1 < BusinessRules.SAVING_ACCOUNT_MIN_AMOUNT_TO_LEFT) {
				throw new BelowMinimumBalanceException(
			...
			try {
				...
				connection.rollback();
				return new ChangesStatus(false,
						"Rolledbacked. Transaction Unsuccessful.");
			} catch (SQLException e1) {
				e1.printStackTrace();
				return new ChangesStatus(false, "Unsuccessful Rolledback.");
			}
